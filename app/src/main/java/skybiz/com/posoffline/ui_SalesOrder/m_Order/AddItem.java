...
public class AddItem extends AsyncTask<Void, Void, String> {
    ...
    String deviceId,RunNo,CurCode,DUD6;
    Double Qty,UnitPrice,HCTax, HCDiscount, HCLineAmt,dQty,DisRate1;
    TelephonyManager telephonyManager;

    public AddItem(Context c, String itemCode, String vQty, String vUnitPrice, String vHCDiscount, String vDisRate1) {
        this.c = c;
        ItemCode = itemCode;
        this.vQty = vQty;
    ...
    private String fnadd(){
        try{
           ...
           // deviceId = telephonyManager.getDeviceId();
            ...
            if(conn!=null && DBStatus.equals("1")){
                if(dQty==0.00){
                    Qty=1.00;
                    if(HCDiscount>0){
                    ...
                    vData = fnadditem1(ItemCode, Qty, UserCode, HCDiscount,DisRate1);
                }else{
                    if(vQty.equals("1")){
                        Qty = dQty+1.00;
                    }else {
                ...
                if(dQty==0.00){
                    ...
                    vData = fnadditem(ItemCode, Qty, deviceId,HCDiscount,DisRate1);
                }else{
                    if(vQty.equals("1")){
                        Qty = dQty+1.00;
                    }else {
    ...
    //add new item
    private String fnadditem(String ItemCode, Double Qty, String deviceId, Double HCDiscount, Double DisRate1){
        Double HCTax, HCLineAmt,TaxRate1,UnitPrice,UOMPrice1,UOMPrice2,UOMPrice3,UOMPrice4,UOMFactor1,UOMFactor2,UOMFactor3,UOMFactor4,FactorQty;
    ...
    //add new item
    public String fnadditem1(String ItemCode, Double Qty, String deviceId, Double HCDiscount, Double DisRate1){
        Double HCTax, HCLineAmt,TaxRate1,UnitPrice,UOMPrice1,UOMPrice2,UOMPrice3,UOMPrice4,UOMFactor1,UOMFactor2,UOMFactor3,UOMFactor4,FactorQty;
        String DetailTaxCode,Description,DefaultUOM,UOM,UOM1,UOM2,UOM3,UOM4,RetailTaxCode,PurchaseTaxCode,SalesTaxCode;
        HCTax         =0.00;
        HCLineAmt     =0.00;
        ...
        try {
            ...
            if (conn != null) {
                String sql = "Select DefaultUOM,Description,UOM,UOM1,UOM2,UOM3,UOM4,IFNULL(UOMFactor1,0) as UOMFactor1,IFNULL(UOMFactor2,0) as UOMFactor2, IFNULL(UOMFactor3,0) as UOMFactor3, IFNULL(UOMFactor4,0) as UOMFactor4," +
                        " IFNULL(UOMPrice1,0) as UOMPrice1, IFNULL(UOMPrice2,0) as UOMPrice2, IFNULL(UOMPrice3,0) as UOMPrice3, IFNULL(UOMPrice4,0) UOMPrice4,RetailTaxCode,PurchaseTaxCode, SalesTaxCode,UnitPrice from stk_master where ItemCode='" + ItemCode + "' ";
                ...
                while (rsItem.next()) {
                    ...
                    RetailTaxCode = rsItem.getString(18);
                }
                UnitPrice = Double.parseDouble(vUnitPrice);
                if (DefaultUOM.equals("0")) {
                    FactorQty = 1.00;
