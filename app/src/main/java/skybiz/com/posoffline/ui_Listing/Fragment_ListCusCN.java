...
import skybiz.com.posoffline.ui_Listing.m_ListCusCN.TrnAdapter;
...
public class Fragment_ListCusCN extends Fragment {
    View view;
   ...
   /* TrnAdapter adapter;
    ...
    private void fnPrint() {
        ...
        PrintList printList=new PrintList(getActivity(),"CusCN",T_ype,DateFrom,DateTo);
        printList.execute();
        refresh();
    }
    ...
    public void refresh() {
        ...
        DownloaderList downloaderList=new DownloaderList(getActivity(),"CusCN",DateFrom,DateTo,rv);
        downloaderList.execute();
        //adapter=new TrnAdapter(getActivity(),lscr,getActivity().getSupportFragmentManager());
        //retCS("","");
    }

    public void retCS(String DateFrom,String DateTo){
       /* String vClause="";
        ...
        db.openDB();
        String vQuery="select H.Doc1No,H.D_ateTime,H.HCNetAmt," +
                "IFNULL(H.SynYN,'0') as SynYN,Sum(D.Qty) as Qty, C.CusName " +
                "from stk_cus_inv_hd H inner join stk_cus_inv_dt D on D.Doc1No=H.Doc1No " +
                "left join customer C ON H.CusCode=C.CusCode " +
                "where H.DocType='CusCN' "+vClause+" Group By H.Doc1No Order By H.RunNo Desc";
