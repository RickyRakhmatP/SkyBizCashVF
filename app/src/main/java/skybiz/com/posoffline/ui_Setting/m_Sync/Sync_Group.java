...
public class Sync_Group extends AsyncTask<Void,Integer,String> {
    Context c;
    ProgressBar bar;
    ...
    public Sync_Group(Context c, ProgressBar bar) {
        this.c = c;
        this.bar = bar;
    }
    @Override
    ...
    protected void onProgressUpdate(Integer... values) {
        ...
        if (this.bar != null) {
            ...
            ((Sync)c).SetProgressGroup(T_ypeSync+" "+values[0] +" of "+ totalrows);
        }
    ...
    protected void onPostExecute(String result) {
        ...
        if(result.equals("error")){
            Toast.makeText(c,"Unsuccessfull, no data group retrieve", Toast.LENGTH_SHORT).show();
        }else{
            Toast.makeText(c,"Successfull, inserting data group ", Toast.LENGTH_SHORT).show();
        }
    }
        ...
        try{
            ...
            if(DBStatus.equals("2")) {
                ...
                if (conn == null) {
                    ...
                    String qTotal = "select count(*)as totalrows from stk_group "+vClause+" ";
                    ...
                    String qTotalU = "select count(*)as totalrows from stk_uom ";
                    Statement stmtTotalU = conn.createStatement();
                    stmtTotalU.execute(qTotalU);
                    ResultSet rsTotalU = stmtTotalU.getResultSet();
                    while (rsTotalU.next()) {
                        totalrows = rsTotalU.getInt(1);
                    }
