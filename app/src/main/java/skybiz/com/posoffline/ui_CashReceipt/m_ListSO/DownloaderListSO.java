...
import skybiz.com.posoffline.ui_CashReceipt.m_Item.ItemParser;
...
public class DownloaderListSO extends AsyncTask<Void,Void,String> {
    Context c;
    RecyclerView rv;
    String IPAddress,UserName,Password,DBName,Port,URL,z,DBStatus,ItemConn;
    String CurCode;
    Fragment_ListSO fragment_listSO;
    public DownloaderListSO(Context c,  RecyclerView rv,Fragment_ListSO fragment_listSO) {
        this.c = c;
        this.rv = rv;
    ...
    protected void onPostExecute(String result) {
        ...
        if(result==null){
            ...
            ListSOParser p=new ListSOParser(c,result,rv,fragment_listSO);
            p.execute();
        }
    }
    ...
    private String downloadData(){
        try {
            ...
            if(DBStatus.equals("1")) {
                ...
                if (conn != null) {
                    String query = "select Doc1No,Doc2No,Attention from stk_sales_order_hd " +
                ...
                while(rsData.moveToNext()){
                    ...
                    row.put("Attention",rsData.getString(2));
                    results.put(row);
                }
                db.closeDB();
                return results.toString();
            }else if(DBStatus.equals("2")){
                String sql="select Doc1No,Doc2No,Attention from stk_sales_order_hd" +
                        " where Status='Waiting' and Doc2No NOT IN('9999','') ";
                Log.d("query",sql);
