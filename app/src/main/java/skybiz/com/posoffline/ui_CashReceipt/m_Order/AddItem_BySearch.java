        ...
        try{
           ...
           // deviceId = telephonyManager.getDeviceId();
            ...
            String vCheck="select RunNo, Qty, HCUnitCost," +
                    " HCDiscount, DisRate1, AnalysisCode2 from cloud_cus_inv_dt where" +
    ...
    private String addItem(String ItemCode, Double Qty){
        ...
        try {
            ...
            String sql = "Select '' as RunNo,M.DefaultUOM, M.Description, M.UOM, M.UOM1," +
                    ...
                    " M.UnitPrice, M.AlternateItem, M.HCDiscount," +
                    " M.DisRate1, "+qPoint+", IFNULL(M.MSP,0)as MSP, " +
                    " IFNULL(M.UnitCost,0)as UnitCost, M.ItemGroup "+
                    " from stk_master M left join ret_point P ON M.ItemCode=P.Item " +
                    " where M.ItemCode='" + ItemCode + "' Group By M.ItemCode";
            Log.d("QUERY SEARCH",sql);
            ...
            if (DBStatus.equals("0")) {
                ...
                while (rsData.moveToNext()) {
                    ...
                    UnitCost        =rsData.getDouble(25);
                    vItemGroup      =rsData.getString(26);
                    i++;
                }
                ...
                if (conn != null) {
                    ...
                    while (rsItem.next()) {
                        ...
                        UnitPrice=Double.parseDouble(vUnitPrice);
                        HCDiscount = rsItem.getDouble(22);
                        DisRate1 = rsItem.getDouble(23);
                        Point = rsItem.getString(24);
                        MSP = rsItem.getDouble(25);
                        UnitCost = rsItem.getDouble(26);
                        vItemGroup = rsItem.getString(27);
                        i++;
                    }
                }
            ...
            if(i==0){
                String sql2 ="select U.UOM, U.FactorQty, U.UnitPrice," +
                        " M.ItemCode, M.Description, M.RetailTaxCode, " +
                        " IFNULL(M.HCDiscount,0)as HCDiscount, IFNULL(M.DisRate1,0)as DisRate1, '0' as BestPriceYN , " +
                        " IFNULL(U.MSP,0)as MSP, IFNULL(U.MAXSP,0)as MAXSP, IFNULL(U.MPP,0)as MPP " +
                        " From stk_master_uom U " +
                        " inner join stk_master M on U.ItemCode=M.ItemCode " +
                        " where U.ChildCode='"+ItemCode+"' ";

                if(conn!=null) {
                    Statement stmt2 = conn.createStatement();
                    stmt2.execute(sql2);
                    ResultSet rsData = stmt2.getResultSet();
                    while (rsData.next()) {
                        UOM             = rsData.getString(1);
                        FactorQty       = rsData.getDouble(2);
                        UnitPrice       = rsData.getDouble(3);
                        ItemCode        = rsData.getString(4);
                        Description     = Encode.setChar(EncodeType, rsData.getString(5));
                        RetailTaxCode   = rsData.getString(6);
                        MSP             = rsData.getDouble(10);
                        i++;
                    }
                }else{
                    Cursor rsData=db.getQuery(sql2);
                    while (rsData.moveToNext()) {
                        UOM             = rsData.getString(0);
                        FactorQty       = rsData.getDouble(1);
                        UnitPrice       = rsData.getDouble(2);
                        ItemCode        = rsData.getString(3);
                        Description     = Encode.setChar(EncodeType, rsData.getString(4));
                        RetailTaxCode   = rsData.getString(5);
                        MSP             = rsData.getDouble(9);
                        i++;
                    }
                }
            }
            ...
            String vSQLInsert = "INSERT INTO cloud_cus_inv_dt (Doc1No, N_o, ItemCode, Description, " +
                    ...
                    "Point, ServiceChargeYN, UnitCost, LineNo," +
                    " ItemGroup)" +
                    "VALUES(" +
