...
public class DownloaderCustomer extends AsyncTask<Void,Void,String> {
    ...
    public DownloaderCustomer(Context c, String SearchBy ,String Keyword, RecyclerView rv) {
        this.c = c;
    ...
    protected void onPostExecute(String result) {
        ...
        if(result==null){
            ...
            CustomerParser p=new CustomerParser(c,result,rv);
            p.execute();
        }
    }
        ...
        try {
            ...
            if(DBStatus.equals("1")){
                ...
                if (conn != null) {
                    String vClause1="";
                    if(SearchBy.equals("By CusCode")){
                        vClause1=" and C.CusCode like '%"+Keyword+"%' ";
                    }else if(SearchBy.equals("By CusName")){
                        vClause1=" and C.CusName like '%"+Keyword+"%' ";
                    }
                    String sql ="Select C.CusCode,C.CusName,C.TermCode," +
                            " C.D_ay,C.SalesPersonCode,C.Address," +
                            " T.TermDesc, C.CategoryCode " +
                            " from customer C left join stk_term T " +
                            " on C.TermCode=T.TermCode where C.FinCatCode='B55' "+vClause1+" ";
                   // Log.d("QUERY",sql);
               ...
               // z="success";
                ...
                String vQuery="Select CusCode,CusName,TermCode," +
                        "D_ay, SalesPersonCode,Address, '' as TermDesc," +
                        "CategoryCode " +
               ...
               // Log.d("TOTAL ROWS",vQuery+String.valueOf(rsData.getCount()));
                ...
                while(rsData.moveToNext()){
                    ...
                    row2.put("Address",rsData.getString(5));
                    row2.put("TermDesc",rsData.getString(6));
                    row2.put("CategoryCode",rsData.getString(7));
                    results2.put(row2);
                }
                db.closeDB();
                ...
                String vQuery="Select CusCode,CusName,TermCode,D_ay, SalesPersonCode, Address, '' as TermDesc," +
                        "CategoryCode " +
                        "from customer where CusCode<>'' "+vClause+" ";
